...
A same-class `@name.setter` on a method named `name` permits writes; `@Base.name.setter` also explicitly declares a local setter. `@property` alone permits annotation-only declarations but does not permit writes. Bare `property`, `staticmethod`, `classmethod`, and their `builtins.` spellings are recognized syntactically. Static methods, class methods, and `__new__` are excluded. Methods must be direct statements of their class body; nested functions are not followed, but nested classes are checked independently. Import aliases, decorator rebinding, receiver rebinding, and runtime property replacement are not resolved.
