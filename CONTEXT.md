...
A policy requiring public instance data to be explicit through same-class class-body annotations or properties, with underscore-prefixed storage for internal state. It flags direct public receiver stores in instance methods without an instance-data declaration or same-class setter. Public data names do not start with an underscore, independently of the owning class's name. Recognizable ClassVar and InitVar annotations do not declare instance data; a local getter-only property takes precedence over an annotation. Inherited declarations and runtime field semantics are not resolved. Ruff SLF001 supplies the companion rule for external backing-field access.
